...
import kz.shprot.office.fc.hslf.record.Sound;
...
public final class SoundData
{
    ...
    public static SoundData[] find(Document document)
    {
        ...
        kz.shprot.office.fc.hslf.record.Record[] ch = document.getChildRecords();
        for (int i = 0; i < ch.length; i++)
        {
            if (ch[i].getRecordType() == RecordTypes.SoundCollection.typeID)
            {
                RecordContainer col = (RecordContainer)ch[i];
                kz.shprot.office.fc.hslf.record.Record[] sr = col.getChildRecords();
