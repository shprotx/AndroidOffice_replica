...
import kz.shprot.office.fc.ddf.EscherBinaryTagRecord;
...
import kz.shprot.office.fc.hslf.record.CString;
import kz.shprot.office.fc.hslf.record.ColorSchemeAtom;
import kz.shprot.office.fc.hslf.record.EscherTextboxWrapper;
import kz.shprot.office.fc.hslf.record.ExtendedParagraphAtom;
...
import kz.shprot.office.fc.hslf.record.SheetContainer;
import kz.shprot.office.fc.hslf.record.StyleTextPropAtom;
import kz.shprot.office.fc.hslf.record.TextBytesAtom;
...
public abstract class Sheet
{
        ...
        for (int i = 0; i < wrappers.length; i++)
        {
            ...
            if (s2 != s1)
            {
                ...
                for (int j = i - 1; j >= 0; j--)
                {
                    ...
                    {
                        kz.shprot.office.fc.hslf.record.Record[] records = wrappers[j].getChildRecords();
     ...
     */
    protected static void findTextRuns(kz.shprot.office.fc.hslf.record.Record[] records, Vector found)
    {
        ...
        for (int i = 0, slwtIndex = 0; i < (records.length - 1); i++)
        {
            if (records[i] instanceof TextHeaderAtom)
            {
                ...
                if (trun != null)
                {
                    ...
                    kz.shprot.office.fc.hslf.record.Record[] recs = new kz.shprot.office.fc.hslf.record.Record[lst.size()];
                    lst.toArray(recs);
                    trun._records = recs;
     ...
     *
     * @param type  type of placeholder to search. See {@link kz.shprot.office.fc.hslf.record.OEPlaceholderAtom}
     * @return  <code>TextShape</code> or <code>null</code>
     */
