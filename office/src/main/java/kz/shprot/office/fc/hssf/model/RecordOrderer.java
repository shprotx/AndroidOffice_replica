...
import kz.shprot.office.fc.hssf.record.ArrayRecord;
import kz.shprot.office.fc.hssf.record.BOFRecord;
import kz.shprot.office.fc.hssf.record.BlankRecord;
import kz.shprot.office.fc.hssf.record.BoolErrRecord;
...
import kz.shprot.office.fc.hssf.record.DrawingSelectionRecord;
import kz.shprot.office.fc.hssf.record.EOFRecord;
import kz.shprot.office.fc.hssf.record.FeatRecord;
import kz.shprot.office.fc.hssf.record.FormulaRecord;
...
import kz.shprot.office.fc.hssf.record.LabelRecord;
import kz.shprot.office.fc.hssf.record.LabelSSTRecord;
import kz.shprot.office.fc.hssf.record.NumberRecord;
...
import kz.shprot.office.fc.hssf.record.RKRecord;
...
import kz.shprot.office.fc.hssf.record.SharedFormulaRecord;
import kz.shprot.office.fc.hssf.record.TableRecord;
...
final class RecordOrderer {
	...
	private static boolean isProtectionSubsequentRecord(Object rb) {
		...
		if (rb instanceof kz.shprot.office.fc.hssf.record.Record) {
			kz.shprot.office.fc.hssf.record.Record record = (kz.shprot.office.fc.hssf.record.Record) rb;
	...
	private static int findInsertPosForNewCondFormatTable(List<RecordBase> records) {
		...
		for (int i = records.size() - 2; i >= 0; i--) { // -2 to skip EOF record
			...
			kz.shprot.office.fc.hssf.record.Record rec = (kz.shprot.office.fc.hssf.record.Record) rb;
			switch (rec.getSid()) {
	...
	private static int findDataValidationTableInsertPos(List<RecordBase> records) {
		...
		while (i > 0) {
			...
			if (isDVTPriorRecord(rb)) {
				kz.shprot.office.fc.hssf.record.Record nextRec = (kz.shprot.office.fc.hssf.record.Record) records.get(i + 1);
	...
	private static boolean isDVTPriorRecord(RecordBase rb) {
		...
		short sid = ((kz.shprot.office.fc.hssf.record.Record)rb).getSid();
		switch(sid) {
			case WindowTwoRecord.sid:
