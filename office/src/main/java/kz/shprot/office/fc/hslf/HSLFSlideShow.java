...
package kz.shprot.office.fc.hslf;

import android.util.Log;

import kz.shprot.office.constant.EventConstant;
import kz.shprot.office.fc.fs.filesystem.CFBFileSystem;
import kz.shprot.office.fc.fs.filesystem.Property;
import kz.shprot.office.fc.hslf.blip.Metafile;
import kz.shprot.office.fc.hslf.exceptions.CorruptPowerPointFileException;
import kz.shprot.office.fc.hslf.exceptions.EncryptedPowerPointFileException;
import kz.shprot.office.fc.hslf.model.Picture;
import kz.shprot.office.fc.hslf.record.CurrentUserAtom;
import kz.shprot.office.fc.hslf.record.ExOleObjStg;
import kz.shprot.office.fc.hslf.record.PersistPtrHolder;
import kz.shprot.office.fc.hslf.record.PersistRecord;
import kz.shprot.office.fc.hslf.record.UserEditAtom;
import kz.shprot.office.fc.hslf.usermodel.ObjectData;
...
public final class HSLFSlideShow/* extends POIDocument*/ {
     ...
     */
    public HSLFSlideShow(IControl control, String filepath) throws IOException {
        this.control = control;
        cfbFS = new CFBFileSystem(new FileInputStream(filepath));
    ...
    private kz.shprot.office.fc.hslf.record.Record[] read(/*byte[] docstream, */int usrOffset) {
...
//            UserEditAtom usr = (UserEditAtom)Record.buildRecordAtOffset(docstream, usrOffset);
            ...
            UserEditAtom usr = (UserEditAtom) kz.shprot.office.fc.hslf.record.Record.buildRecordAtOffset(data, 0, usrOffset);
...
//                psrOffset);
            ...
            PersistPtrHolder ptr = (PersistPtrHolder) kz.shprot.office.fc.hslf.record.Record.buildRecordAtOffset(data, 0, psrOffset);
        ...
        Arrays.sort(a);
        kz.shprot.office.fc.hslf.record.Record[] rec = new kz.shprot.office.fc.hslf.record.Record[lst.size()];
        for (int i = 0; i < a.length; i++) {
            Integer offset = a[i];
//            rec[i] = Record.buildRecordAtOffset(docstream, offset.intValue());
            byte[] data = _docProp.getRecordData(offset.intValue());
            rec[i] = kz.shprot.office.fc.hslf.record.Record.buildRecordAtOffset(data, 0, offset.intValue());
...
//                                out.write(pict.getData());
     ...
     */
    public synchronized int appendRootLevelRecord(kz.shprot.office.fc.hslf.record.Record newRecord) {
        int addedAt = -1;
        kz.shprot.office.fc.hslf.record.Record[] r = new kz.shprot.office.fc.hslf.record.Record[_records.length + 1];
        boolean added = false;
     ...
     */
    public kz.shprot.office.fc.hslf.record.Record[] getRecords() {
        return _records;
    }
    ...
    public PictureData[] getPictures() {
        if (_pictures == null) {
            try {
                ...
                Log.d("exception", "HSLFSlideShow getPictures OutOfMemoryError: " + e);
    ...
    public void dispose() {
        ...
        if (_records != null) {
            for (kz.shprot.office.fc.hslf.record.Record rec : _records) {
                rec.dispose();
            }
            _records = null;
        }
