...
package kz.shprot.office.fc.hssf.eventmodel;
 ...
 * this version sends {} messages to
...
public final class EventRecordFactory {
	 ...
	 */
	private boolean processRecord(kz.shprot.office.fc.hssf.record.Record record) {
	...
	public void processRecords(InputStream in) throws RecordFormatException {
		kz.shprot.office.fc.hssf.record.Record last_record = null;

		RecordInputStream recStream = new RecordInputStream(in);

		while (recStream.hasNextRecord()) {
			recStream.nextRecord();
			kz.shprot.office.fc.hssf.record.Record[] recs = RecordFactory.createRecord(recStream);   // handle MulRK records
			if (recs.length > 1) {
				...
				kz.shprot.office.fc.hssf.record.Record record = recs[ 0 ];
