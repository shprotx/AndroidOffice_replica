...
import kz.shprot.office.fc.EncryptedDocumentException;
import kz.shprot.office.fc.hssf.eventusermodel.HSSFEventFactory;
import kz.shprot.office.fc.hssf.eventusermodel.HSSFListener;
import kz.shprot.office.fc.hssf.record.crypto.Biff8EncryptionKey;
...
public final class RecordFactoryInputStream {
	...
	private static final class StreamEncryptionInfo {
		...
		private final kz.shprot.office.fc.hssf.record.Record _lastRecord;
		private final boolean _hasBOFRecord;

		public StreamEncryptionInfo(RecordInputStream rs, List<kz.shprot.office.fc.hssf.record.Record> outputRecs) {
			kz.shprot.office.fc.hssf.record.Record rec;
			rs.nextRecord();
		 ...
		 * if stream was empty
		 */
		public kz.shprot.office.fc.hssf.record.Record getLastRecord() {
			return _lastRecord;
		}
	...
	/**
	 * Temporarily stores a group of {@link kz.shprot.office.fc.hslf.record.Record}s, for future return by {@link #nextRecord()}.
	 ...
	 */
	private kz.shprot.office.fc.hssf.record.Record[] _unreadRecordBuffer;
	 ...
	 */
	private kz.shprot.office.fc.hssf.record.Record _lastRecord = null;
	/**
	...
	public RecordFactoryInputStream(InputStream in, boolean shouldIncludeContinueRecords) {
		...
		if (!records.isEmpty()) {
			_unreadRecordBuffer = new kz.shprot.office.fc.hssf.record.Record[records.size()];
	 ...
	 */
	public kz.shprot.office.fc.hssf.record.Record nextRecord() {
	...
	/**
	 * @return the next {@link kz.shprot.office.fc.hslf.record.Record} from the multiple record group as expanded from
	 * a recently read {@link MulRKRecord}. <code>null</code> if not present.
	 */
	private kz.shprot.office.fc.hssf.record.Record getNextUnreadRecord() {
		if (_unreadRecordBuffer != null) {
			int ix = _unreadRecordIndex;
			if (ix < _unreadRecordBuffer.length) {
				kz.shprot.office.fc.hssf.record.Record result = _unreadRecordBuffer[ix];
	 ...
	 */
	private kz.shprot.office.fc.hssf.record.Record readNextRecord() {

		kz.shprot.office.fc.hssf.record.Record record = RecordFactory.createSingleRecord(_recStream);
		_lastRecordWasEOFLevelZero = false;
		...
		if (record instanceof MulRKRecord) {
			kz.shprot.office.fc.hssf.record.Record[] records = RecordFactory.convertRKRecords((MulRKRecord) record);
