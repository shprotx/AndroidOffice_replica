...
import kz.shprot.office.fc.ddf.DefaultEscherRecordFactory;
import kz.shprot.office.fc.ddf.EscherBoolProperty;
import kz.shprot.office.fc.ddf.EscherClientAnchorRecord;
import kz.shprot.office.fc.ddf.EscherClientDataRecord;
...
import kz.shprot.office.fc.ddf.EscherRecordFactory;
import kz.shprot.office.fc.ddf.EscherSerializationListener;
import kz.shprot.office.fc.ddf.EscherSpRecord;
import kz.shprot.office.fc.ddf.EscherSpgrRecord;
import kz.shprot.office.fc.ddf.EscherTextboxRecord;
import kz.shprot.office.fc.hssf.model.AbstractShape;
import kz.shprot.office.fc.hssf.model.CommentShape;
import kz.shprot.office.fc.hssf.model.ConvertAnchor;
import kz.shprot.office.fc.hssf.model.DrawingManager2;
import kz.shprot.office.fc.hssf.model.TextboxShape;
...
import kz.shprot.office.fc.hssf.usermodel.HSSFShapeContainer;
import kz.shprot.office.fc.hssf.usermodel.HSSFShapeFactory;
...
public final class EscherAggregate extends AbstractEscherHolderRecord {
	...
	private Map<EscherRecord, kz.shprot.office.fc.hssf.record.Record> shapeToObj = new HashMap<EscherRecord, kz.shprot.office.fc.hssf.record.Record>();
	//chart records
	 private Map<EscherRecord, List<kz.shprot.office.fc.hssf.record.Record>> chartToObj = new HashMap<EscherRecord, List<kz.shprot.office.fc.hssf.record.Record>>();
	...
	public static int shapeContainRecords(List records, int currentDrawingRecord)
	{
	    ...
	    {
	        kz.shprot.office.fc.hssf.record.Record record = (kz.shprot.office.fc.hssf.record.Record) records.get( currentDrawingRecord + 1 );
	        count = 2;
	        //chart records
            if(record instanceof ObjRecord && ((ObjRecord)record).getSubRecords().get(0) instanceof CommonObjectDataSubRecord)
            {
                ...
                {
                  //chart 
                    List<kz.shprot.office.fc.hssf.record.Record> chartRecordsList = new ArrayList<kz.shprot.office.fc.hssf.record.Record>();
                    currentDrawingRecord += 2;     
                    record = (kz.shprot.office.fc.hssf.record.Record) records.get( currentDrawingRecord );
    ...
    { // TODO - remove this method
        ...
        for (int i = currentIndex + 1; i < max; i++)
        {
            ...
            if (!(rb instanceof kz.shprot.office.fc.hslf.record.Record))
            {
                continue;
            }
	...
	public static EscherAggregate createAggregate( List records, int locFirstDrawingRecord, DrawingManager2 drawingManager )
	{
		...
		loc = locFirstDrawingRecord;
		int shapeIndex = 0;
		agg.shapeToObj = new HashMap<EscherRecord, kz.shprot.office.fc.hssf.record.Record>();
		while ( loc > -1 && loc + 1 < records.size()
				&& (sid( records, loc ) == DrawingRecord.sid || sid( records, loc ) == ContinueRecord.sid))
		{
		    if(!isObjectRecord( records, loc + 1 ))
		    ...
		    kz.shprot.office.fc.hssf.record.Record record = (kz.shprot.office.fc.hssf.record.Record) records.get( loc + 1 );
		    try
		    {
		      //chart records
	            if(record instanceof ObjRecord && ((ObjRecord)record).getSubRecords().get(0) instanceof CommonObjectDataSubRecord)
	            {
	                ...
	                if(commonObjectDataSubRecord.getObjectType() == CommonObjectDataSubRecord.OBJECT_TYPE_CHART)
	                {
	                    ...
	                    List<kz.shprot.office.fc.hssf.record.Record> chartRecordsList = new ArrayList<kz.shprot.office.fc.hssf.record.Record>();
	                    loc += 2;     
	                    record = (kz.shprot.office.fc.hssf.record.Record) records.get( loc );
	                    while(record.getSid() != EOFRecord.sid)
	                    {
	                        chartRecordsList.add(record); 
	...
	public int serialize( int offset, byte[] data )
	{
		...
		for ( int i = 1; i < shapes.size(); i++ )
		{
			...
			kz.shprot.office.fc.hssf.record.Record obj = shapeToObj.get( shapes.get( i ) );
		...
		{
			kz.shprot.office.fc.hssf.record.Record rec = (kz.shprot.office.fc.hssf.record.Record)tailRec.get(i);
	...
	public int getRecordSize() {
		...
		{
			kz.shprot.office.fc.hssf.record.Record r = (kz.shprot.office.fc.hssf.record.Record) iterator.next();
	...
	private void convertRecordsToUserModel(EscherContainerRecord shapeContainer, Object model) 
	{
		for(Iterator<EscherRecord> it = shapeContainer.getChildIterator(); it.hasNext();) 
		{
			...
			else if(r instanceof EscherTextboxRecord) 
			{
				...
				kz.shprot.office.fc.hssf.record.Record obj = shapeToObj.get(tbr);
			...
			else if(r instanceof EscherClientDataRecord && model instanceof HSSFChart) 
            {
                ...
                List<kz.shprot.office.fc.hssf.record.Record> recordsList = chartToObj.get(cdr);
...
//		dg.setLastMSOSPID( 0 );  // populated after all shape id's are assigned.
	...
	{
		return ( (kz.shprot.office.fc.hssf.record.Record) records.get( loc ) ).getSid();
	}
}
