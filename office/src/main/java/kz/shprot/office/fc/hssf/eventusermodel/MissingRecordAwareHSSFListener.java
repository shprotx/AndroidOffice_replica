...
package kz.shprot.office.fc.hssf.eventusermodel;


import kz.shprot.office.fc.hssf.eventusermodel.dummyrecord.LastCellOfRowDummyRecord;
import kz.shprot.office.fc.hssf.eventusermodel.dummyrecord.MissingCellDummyRecord;
import kz.shprot.office.fc.hssf.eventusermodel.dummyrecord.MissingRowDummyRecord;
...
import kz.shprot.office.fc.hssf.record.MulRKRecord;
...
public final class MissingRecordAwareHSSFListener implements HSSFListener {
	...
	public void processRecord(kz.shprot.office.fc.hssf.record.Record record) {
		int thisRow;
		int thisColumn;
		...
		if(expandedRecords != null && expandedRecords.length > 0) {
			for(CellValueRecordInterface r : expandedRecords) {
				childListener.processRecord((kz.shprot.office.fc.hssf.record.Record)r);
			}
		} else {
			childListener.processRecord(record);
		}
